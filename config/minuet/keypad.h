...
static Menu current_menu() {
  ...
  if (minuet::coordinator::get_mode() == minuet::coordinator::Mode::AUTO)
...
class Keypad {
 ...
 private:
  ...
  static constexpr uint32_t KEY_COMBO_DIRECTION_POWER = KEY_POWER | KEY_DIRECTION;
  static constexpr uint32_t KEY_COMBO_POWER_OPEN_CLOSE = KEY_POWER | KEY_COMBO_OPEN_CLOSE;
  static constexpr uint32_t KEY_COMBO_POWER_OPEN_CLOSE_ALIAS = KEY_POWER | KEY_COMBO_OPEN_CLOSE | KEY_DIRECTION;
      ...
      case Menu::ENHANCED:
        if (minuet::fan::is_on()) {
          const auto SPEED_CYCLE = {
              minuet::fan::FAN_SPEED_MINIMAL,
              minuet::fan::FAN_SPEED_LOW,
              minuet::fan::FAN_SPEED_MEDIUM,
              minuet::fan::FAN_SPEED_HIGH,
          };
          const auto it =
              std::find_if(SPEED_CYCLE.begin(), SPEED_CYCLE.end(), [](int speed) { return speed > minuet_fan->speed; });
          const int old_speed = minuet_fan->speed;
          minuet::coordinator::command_manual_fan_speed(it != SPEED_CYCLE.end() ? *it : *SPEED_CYCLE.begin());
          minuet::tone::play_fan_speed(old_speed, minuet_fan->speed);
        } else {
          minuet::coordinator::command_manual_power_on();
          minuet::tone::play("?power_on");
        }
  ...
  void do_auto_temp_change_(int steps) {
    if (minuet::coordinator::command_auto_thermostat_temperature_change(steps)) {
      minuet::tone::play(steps > 0 ? "auto_temp_up" : "auto_temp_down");
    } else {
  ...
  void finish_auto_power_action_() {
    switch (this->pending_auto_power_action_) {
      case 0:
        return;
      case 1:
        minuet::coordinator::command_auto_thermostat_settings(minuet::auto_mode::get_thermostat_default_fan_mode(),
            minuet::auto_mode::get_thermostat_default_fan_speed());
        minuet::tone::play("setting_1");
        break;
      case 2:
        minuet::coordinator::command_auto_thermostat_settings(
            minuet::auto_mode::AutoFanMode::AUTO, minuet::fan::FAN_SPEED_LOW);
        minuet::tone::play("setting_2");
        break;
      case 3:
        minuet::coordinator::command_auto_thermostat_settings(
            minuet::auto_mode::AutoFanMode::OFF, minuet::fan::FAN_SPEED_MINIMAL);
        minuet::tone::play("setting_3");
        break;
      case 4:
        minuet::coordinator::command_auto_thermostat_settings(
            minuet::auto_mode::AutoFanMode::ON, minuet::fan::FAN_SPEED_LOW);
        minuet::tone::play("setting_4");
        break;
      case 5:
        minuet::coordinator::command_auto_thermostat_settings(
            minuet::auto_mode::AutoFanMode::ON, minuet::fan::FAN_SPEED_MEDIUM);
        minuet::tone::play("setting_5");
        break;
      case 6:
        minuet::coordinator::command_auto_thermostat_settings(
            minuet::auto_mode::AutoFanMode::ON, minuet::fan::FAN_SPEED_HIGH);
    ...
    switch (current_menu()) {
      ...
      case Menu::ENHANCED:
        if (minuet_rain_safety_lock_latched->state || !minuet_rain_sensor_enabled->state) {
          ESP_LOGI(minuet::TAG, "Command: rain sensor on");
          minuet_rain_sensor_reset->press();
          minuet::tone::play("?rain_on");
        } else {
          ESP_LOGI(minuet::TAG, "Command: rain sensor off");
  ...
  void do_press_direction_power_() {
    switch (current_menu()) {
      case Menu::STANDARD_AUTO:
      case Menu::STANDARD_MANUAL:
      case Menu::ENHANCED:
        minuet::accessory::dispatch_handler(&minuet::accessory::Accessory::handle_mode);
        break;
      default:
        break;
    }
  ...
  void do_action_use_enhanced_controls_() {
    ...
    ESP_LOGI(minuet::TAG, "Command: keypad controls enhanced");
  ...
  void do_action_use_standard_controls_() {
    // Available in all menu modes.
    ESP_LOGI(minuet::TAG, "Command: keypad controls standard");
  ...
  void do_action_keypad_indicators_toggle_() {
    // Available in all menu modes.
    ESP_LOGI(minuet::TAG, "Command: keypad indicators toggle");
  ...
  void do_action_radio_toggle_() {
    // Available in all menu modes.
#ifdef MINUET_RADIO
    ESP_LOGI(minuet::TAG, "Command: radio toggle");
  ...
  void do_action_radio_pairing_() {
    // Available in all menu modes.
#ifdef MINUET_RADIO
    ESP_LOGI(minuet::TAG, "Command: radio pairing mode");
  ...
  void do_action_power_on_behavior_toggle_() {
    // Available in all menu modes.
    ESP_LOGI(minuet::TAG, "Command: power on behavior toggle");
  ...
  void do_action_manual_safety_lock_toggle_() {
    // Available in all menu modes.
    ESP_LOGI(minuet::TAG, "Command: manual safety lock toggle");
  ...
  void do_action_restart_() {
    // Available in all menu modes.
    ESP_LOGI(minuet::TAG, "Command: restart");
  ...
  void do_action_factory_reset_() {
    // Available in all menu modes.
    ESP_LOGI(minuet::TAG, "Command: factory reset");
    ...
    if (press_duration_ms >= MIN_PRESS_DURATION_MS && press_duration_ms <= MAX_PRESS_DURATION_MS) {
      switch (key_state) {
        ...
        case KEY_COMBO_DIRECTION_DOWN:
          ...
          return;
        case KEY_COMBO_DIRECTION_POWER:
          log_press_("KEY_COMBO_DIRECTION_POWER");
          this->do_press_direction_power_();
          return;
      }
    }
  }
  ...
  bool dispatch_hold_(uint32_t key_state, uint32_t hold_duration_ms) {
    ...
    switch (key_state) {
      ...
      case KEY_COMBO_DIRECTION_POWER:
        if (hold_duration_ms >= MIN_HOLD_ACTION_1_DURATION_MS) {
          log_hold_("KEY_COMBO_DIRECTION_POWER");
...
void Keypad::poll_keys() {
  ...
  const uint32_t key_state_raw = (key_r1_c2 ? KEY_UP : 0) | (key_r1_c3 ? KEY_DOWN : 0) | (key_r1_c4 ? KEY_RAIN : 0) |
  ...
  // The keypad cannot distinguish between KEY_POWER and KEY_DIRECTION when combined with KEY_UP and KEY_DOWN
  // so one combination is treated as an alias for the other.  Interestingly, the 6 key wall remote has
  // extra diodes for its dedicated open/close button and it can distinguish those combinations just fine.
  const uint32_t key_state =
      key_state_raw == KEY_COMBO_POWER_OPEN_CLOSE_ALIAS ? KEY_COMBO_POWER_OPEN_CLOSE : key_state_raw;
